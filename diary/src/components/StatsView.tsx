...
// Mêmes hex que le graphe, validés contre la surface (CVD deutan ΔE 12.0).
...
export function StatsView() {
  ...
  // Les cartes sont ré-agrégées sur les événements filtrés.
  const cards = useMemo(() => {
    ...
    return [...acc.entries()]
      ...
      .map(([name, agg]) => {
        ...
        const wr = g ? (agg.wins / g) * 100 : 0
        const gihWr = gih.get(name) ?? null
        return {
          name,
          events: agg.events,
          ...
          winRate: wr,
          gihWr,
          delta: gihWr == null ? null : wr - gihWr,
        }
      })
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
        ...
        <p className="mb-3 mt-1 text-[11px] leading-relaxed text-slate-500">
          Ton WR est un taux de victoire <strong className="text-slate-400">en match</strong>,
          celui de 17Lands un taux <strong className="text-slate-400">en partie</strong> quand
          la carte est en main (GIH). Les deux ne se comparent pas terme à terme — l'écart est
          un signal de tendance, pas une mesure.
        </p>

        {cards.length === 0 ? (
          <p className="py-6 text-center text-sm text-slate-600">
            Aucune carte jouée sur au moins {MIN_EVENTS_FOR_CARD} événements.
          </p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800 text-left text-[11px] uppercase tracking-wide text-slate-500">
                  ...
                  <th className="px-2 py-1.5 text-right font-medium">GIH 17L</th>
...
/**
 * L'écart porte toujours son signe et une flèche : la couleur ne doit jamais
 * être le seul véhicule de l'information.
 */
function Delta({ value }: { value: number | null }) {
  ...
  return (
    <span style={{ color: positive ? POSITIVE : NEGATIVE }}>
      {positive ? '▲' : '▼'} {positive ? '+' : ''}
      {value.toFixed(1)}
    </span>
  )
