...
| `etl_trophy_draft_picks.py` | Séquences de picks de drafts trophées **mythic**, classés par **rang ladder réel** (feature Draft Practice). POST `/api/trophies/` `ranks:["mythic"]` (EXIGE `LANDS17_COOKIE`) → GET `/data/details?draft_id=` pour le vrai rang (leaderboard `#N` ou percentile, **public, pas de fallback** : un draft sans rang lisible est ignoré) → tri meilleurs d'abord → GET `/data/draft?draft_id=` → picks compacts. **Qualitatif** : `MAX_PER_FORMAT=10` nouveaux/set/format/run (scan rang capé à `DETAIL_SCAN_CAP=60`), draft only. Cron quotidien (`trophy_draft_picks.yml`, 16:30). Alimente `trophy_draft_picks`. Deps : `requests` |
